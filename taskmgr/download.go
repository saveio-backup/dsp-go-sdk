...
func (this *TaskMgr) newDownloadTaskFromDB(id string) (*download.DownloadTask, error) {
	...
	if err != nil {
		log.Errorf("new download task get task from db, get file info failed, id: %s", id)
		return nil, err
	}
	if info == nil {
		log.Warnf("new download task get task from db, recover task get file info is nil, id: %v", id)
		return nil, nil
	}
	...
	if err != nil {
		log.Errorf("new download task get task from db, set task session: %s", err)
		return nil, err
	}
