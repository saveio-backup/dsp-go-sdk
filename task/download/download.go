...
func (this *DownloadTask) internalDownload() error {
	...
	_ = this.SetFilePath(fullFilePath)
	...
	if !this.IsInOrder() {
		...
		if this.Mgr.IsClient() &&
			...
			if len(checkFileList) == 0 || checkFileList[0] != this.GetFileHash() {
				this.EmitProgress(types.TaskDownloadCheckingFileFailed)
				return sdkErr.New(sdkErr.CHECK_FILE_FAILED, "file hash not match")
			}
...
func (this *DownloadTask) receiveBlockNoOrder(peerAddrWallet []string) error {
	...
	for {
		...
		case value, ok := <-this.GetTaskNotify():
			...
			getBlock := this.Mgr.Fs().EncodedToBlockWithCid(value.Block, value.Hash)
			blockCid := getBlock.Cid().String()
			if blockCid != value.Hash {
				log.Warnf("receive a unmatched hash block %s %s", blockCid, value.Hash)
			}
			if blockCid == this.GetFileHash() && this.Mgr.IsClient() {
				if !isFileEncrypted {
					_ = this.Mgr.Fs().SetFsFilePrefix(this.GetFilePath(), prefix)
				} else {
					_ = this.Mgr.Fs().SetFsFilePrefix(this.GetFilePath(), "")
				}
			}
			dagLinks, err := this.Mgr.Fs().GetBlockLinksByDAG(getBlock)
			if err != nil {
				return err
			}
...
func rankByValue(m map[string]int64) PairList {
...
func (this *DownloadTask) writeBlockToFile(hasCutPrefix *bool, prefix string, isFileEncrypted bool, block blocks.Block, value *types.BlockResp) error {
	var fileHandler *os.File
	var filePath string
	if this.Mgr.IsClient() {
		var err error
		fileHandler, err = createDownloadFile(this.Mgr.Config().FsFileRoot, this.GetFilePath())
		log.Debugf("create file not in dir: %s", this.GetFilePath())
		if err != nil {
			log.Errorf("createFileErr: %s", err)
			return sdkErr.NewWithError(sdkErr.CREATE_DOWNLOAD_FILE_FAILED, err)
		}
		defer func() {
